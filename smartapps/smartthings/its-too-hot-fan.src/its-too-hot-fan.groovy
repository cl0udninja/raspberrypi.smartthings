 ...
 */
definition(
    name: "It's Too Hot (Fan)",
    namespace: "smartthings",
    author: "SmartThings",
    description: "Monitor the temperature and when it rises above your setting get a text and/or turn on a fan or additional appliance.",
...
preferences {
	...
	section("Turn on a fan...") {
		input "switch1", "capability.switchLevel", required: false
	}
    section("Set level to...") {
		input "fanLevel", "number", range:"(1..99)", title: "Fan level?"
	}
}
...
def temperatureHandler(evt) {
	...
	if (evt.doubleValue >= tooCold) {
		...
		if (alreadySentSms) {
			...
			log.debug "Temperature rised above $tooCold:  sending SMS and activating $mySwitch"
